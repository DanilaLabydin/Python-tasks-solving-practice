...
REQUIREMENTS = {'left': False, 'right': True, }
...
def check_group_requirements(row, side, position, pass_aisle, free_row_indexes):
    if (side == 'left' and pass_aisle is True) or (side == 'right' and  pass_aisle is False):
        return False
...
def check_free_space_for_group(current_seats_map, num, side, position):
    ...
    for row in current_seats_map:
        ...
        print(f'side: {side} - position: {position} - pass_aisle: {pass_aisle} - free_indexes: {free_row_indexes}')
        group_req = check_group_requirements(row, side, position, pass_aisle, free_row_indexes)
        print(group_req)
