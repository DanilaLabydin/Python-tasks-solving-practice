...
def get_verbs_list(file):
    ...
    try:
        with open(file) as f:
            ...
            for row in csv_f:
                rows.append([row['first_form'], row['second_form'], row['third_form'], row['translate']])
            return rows
...
def main():
    ...
    # read the special file to store verbs into a list and mix it
    verbs_list = get_verbs_list(arguments.verbs_filename)

    # display the greeting message that contains the rules of program and store the number of verb to practice
    greeting()
